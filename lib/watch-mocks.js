...
function watchMockDirectories (config) {
    ...
    if (config.watch) {
        ...
        let loaded = false;
        const watcherReady = () => {
            if(ready && loaded && Object.keys(loaded).length === Object.keys(ready).length) {
                watcher.on('all', (...args) => {
                    reloadMock(args[0], args[1])
                    .then(() => {
                        config.emit('mock-updated', args[1]);
                    })
                });    
            }
        };
        ...
        watcher.on('ready', () => {
            ready = watcher.getWatched();
            watcherReady();
        });

        config.on('loaded', () => {
            loaded = watcher.getWatched();
            watcherReady();
        });
