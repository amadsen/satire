...
const proxyTemplateHandlers = {
    'function': (template) => (pattern, req) => template(pattern, req),
    'string': (template) => (pattern, req) => ({
        url: req.url.replace(pattern, template)
    }),
    'object': (template) => {
        if (template.module) {
            const definition = require(template.module)(template);
            return proxyTemplateHandlers[type(definition)](definition);
        }
    }
};
...
const initProxyAPIs = (config) => {
    const proxyPatterns = Object.keys(config.proxyAPIs || {})
        ...
        .map((src) => {
            ...
            const [err, proxyFn] = trike(proxyTemplateHandlers[type(definition)], definition);
            if (!proxyFn) {
