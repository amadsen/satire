...
test('Should support proxying of HTTP server', (suite) => {
    ...
    suite.test('should work with module and function proxyAPIs', (t) => {
        const port = 8091;
        const expectedProxyConfig = {
            port,
            mocks: [
            ...
            proxyAPIs: {
                '/proxy/': {
                    module: './support/function-proxy.js',
                    template: 'http://127.0.0.1:8091/'
                }
            },
            from: __dirname,
            _: { errors: [] }
        ...
        s.on('json-test', () => {
            request(
                `http://127.0.0.1:${port}/proxy/test.json`,
                {
                    headers: {
                        'x-proxy-prefix': 'json/'
                    }
